...
import Junkyard from '../models/Junkyard';
...
router.get('/', async (req, res) => {
  try {
    const junkyards = await Junkyard.find();
    res.json(junkyards);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    const junkyard = await Junkyard.findById(req.params.id);
    if (!junkyard) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const junkyard = new Junkyard(req.body);
...
router.put('/:id', async (req, res) => {
  try {
    const updatedJunkyard = await Junkyard.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedJunkyard = await Junkyard.findByIdAndDelete(req.params.id);
    if (!deletedJunkyard) {
      return res.status(404).json({ message: 'Junkyard not found' });
    }
